...
impl JiraDatabase {
    ...
    pub fn create_story(&self, story: Story, epic_id: u32) -> Result<u32> {
        ...
        let epic = db_state
            .epics
            .get_mut(&epic_id)
            .ok_or(anyhow!("Couldn't find epic_id {}", epic_id))?;
        let new_id = db_state.last_item_id + 1;
    ...
    pub fn delete_epic(&self, epic_id: u32) -> Result<()> {
        ...
        let epic = db_state
            ...
            .get(&epic_id)
            .ok_or(anyhow!("Couldn't find epic_id {}", epic_id))?;
        epic.stories.iter().for_each(|x| {
            db_state.stories.remove(x);
        });
    ...
    pub fn delete_story(&self, epic_id: u32, story_id: u32) -> Result<()> {
        ...
        let idx = epic
            ...
            .ok_or(anyhow!(
                "Couldn't find story_id {} in epic {}",
                story_id,
                epic_id
            ))?;
        epic.stories.remove(idx);
    ...
    pub fn update_story_status(&self, story_id: u32, status: Status) -> Result<()> {
        ...
        let story = db_state
            .stories
            .get_mut(&story_id)
            .ok_or(anyhow!("Couldn't find story_id {}", story_id))?;
        story.status = status;
...
impl Database for JSONFileDatabase {
    ...
    fn write_db(&self, db_state: &DBState) -> Result<()> {
        let db_json = serde_json::to_string(db_state)?;
        fs::write(&self.file_path, &db_json)?;
        Ok(())
    }
}
